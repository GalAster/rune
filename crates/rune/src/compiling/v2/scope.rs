...
pub struct Scope {
    ...
    locals: HashMap<Box<str>, (Span, ValueId)>,
}
...
impl Stack {
    ...
    pub fn declare(&mut self, span: Span, name: &str, value: ValueId) -> Result<(), CompileError> {
    ...
    pub fn get(
        ...
        for scope in self.stack.iter().rev() {
            if let Some((var_span, value_id)) = scope.locals.get(name).copied() {
                visitor.visit_variable_use(self.source_id, var_span, span);
                return Ok(value_id);
            }
        }
